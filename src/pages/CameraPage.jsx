...
const DIAMONDS = [
  ...
  { size: 360, duration: 70, delay: -7.8, opacity: 0.28 },
  { size: 400, duration: 90, delay:  -20, opacity: 0.18 },
];
  ...
  return (
    <div className={`camera ${mounted ? "camera--mounted" : ""}`}>
      ...
      <nav className="camera__nav">
        <div className="camera__nav-left">
          <span className="camera__logo">SKINSTRIC</span>
          ...
          <span className="camera__nav-label">INTRO</span>
          <svg width="4" height="17" viewBox="0 0 4 17" fill="none" aria-hidden="true">
        ...
        <button className="camera__enter-code">ENTER CODE</button>
      </nav>

      {/* Center diamond + icon */}
      <div className="camera__center">
        <div className="camera__icon-wrap">
          <div className="camera__diamonds" aria-hidden="true">
            {DIAMONDS.map((d, i) => (
              <div
                key={i}
                className="camera__diamond"
                style={{
                  width:      d.size,
                  height:     d.size,
                  marginTop:  -(d.size / 2),
                  marginLeft: -(d.size / 2),
                  opacity:    d.opacity,
                  animation:  `cameraRotateCW ${d.duration}s linear ${d.delay}s infinite`,
                }}
              />
            ))}
          </div>
        ...
        <p className="camera__status">SETTING UP CAMERA ...</p>
      </div>

      {/* Bottom tips */}
      <div className="camera__tips">
        <p className="camera__tips-title">TO GET BETTER RESULTS MAKE SURE TO HAVE</p>
        <div className="camera__tips-list">
          <span className="camera__tip"><span className="camera__tip-diamond">&#9671;</span> NEUTRAL EXPRESSION</span>
          <span className="camera__tip"><span className="camera__tip-diamond">&#9671;</span> FRONTAL POSE</span>
          <span className="camera__tip"><span className="camera__tip-diamond">&#9671;</span> ADEQUATE LIGHTING</span>
        </div>
      </div>
